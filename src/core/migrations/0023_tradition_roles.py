...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for role in orm.Role.objects.all():
            for field in ('tradition', 'corporation', 'crime'):
                if getattr(role, field) is not None:
                    tradition = getattr(role, field)
                    orm.TraditionRole.objects.create(
                        tradition=tradition,
                        role=role,
                        is_approved=True,
                        level=tradition.master == role and 'master' or 'ordinal',
                    )
