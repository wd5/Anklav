...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for tradition in orm.Tradition.objects.all():
            members_count = orm.TraditionRole.objects.filter(tradition=tradition, is_approved=True).exclude(level='master').count()
            tradition.mana = sum(xrange(members_count + 1))
            tradition.save()
